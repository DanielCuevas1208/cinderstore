module Cinderstore
  ...
  class Wal
    ...
    #
    # When `checksums` is false the record carries a zero CRC32. The store
    # skips the checksum work and accepts corruption without notice.
    ...
    # preserved. When `verify` is false the CRC32 is read but not checked.
    def self.recover(path : String, mem : MemTable, base_seq : Int64, verify : Bool = true) : Int64
      max_seq = base_seq
