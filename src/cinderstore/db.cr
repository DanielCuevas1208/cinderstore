...
module Cinderstore
  ...
  # merges overflowing levels into deeper levels and drops stale data.
  # Reads merge the memtable and all tables, which makes every view
  # consistent with the write order.
  class DB
    ...
    class Config
      ...
      # Number of tables in a deep level that trigger a merge into the next.
    ...
    class Stats
      getter tables : Int32
      # Table count per level. Index zero is level 0.
    ...
    # Merges tables from overflowing levels into deeper levels.
    def compact : Nil
      @flush_lock.synchronize do
        do_compact
      end
    ...
    private def trigger_compact : Nil
      ...
      needs = @lock.synchronize { select_compaction_level(force: false) }
      return unless needs
    ...
    private def do_compact : Nil
      loop do
        level = @lock.synchronize do
          return if @closed
          select_compaction_level(force: true)
        end
        break unless level
    ...
    #
    # A background pass compacts level 0 only past its threshold. An
    # explicit pass also merges a level-0 pair, so `compact` always makes
    # progress when two tables exist. Deeper levels compact when they grow
    # past `l1_compact_threshold`.
    private def select_compaction_level(force : Bool) : Int32?
      l0 = @levels[0].size
      return 0 if l0 >= @config.l0_compact_threshold
      return 0 if force && l0 >= 2
      (1...@levels.size).each do |i|
        next unless i + 1 < @config.max_level
    ...
    # next level. Outputs land in the next level. Tables that do not
    # overlap the source ranges stay in place, so the merge rewrites only
    # the tables it must touch. Returns false when nothing can be merged.
    private def compact_level(level : Int32) : Bool
      sources, overlaps, drop_tombstones = @lock.synchronize do
        return false if @closed
        src = @levels[level]
        tgt = @levels[level + 1]? || ([] of TableRef)
        ovl = tgt.select { |ref| overlaps_any?(ref, src) }
        deepest = deepest_level
        drop = deepest.nil? || deepest.not_nil! <= level + 1
        {src, ovl, drop}
      end
    ...
    # Returns the index of the deepest level that holds a table, or nil.
    private def deepest_level : Int32?
      (@levels.size - 1).downto(0) do |i|
